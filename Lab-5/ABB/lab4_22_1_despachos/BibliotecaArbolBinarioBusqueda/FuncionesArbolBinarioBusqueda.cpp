...
void insertarRecursivo(NodoArbolBinarioBusqueda *&raiz,const ElementoArbolBinarioBusqueda &elemento) {
    if (esNodoVacio(raiz)) {
        ...
        int cmp = comparaElementos(raiz->elemento.year,elemento.year);
        if (cmp==1) {            // raiz mayor: insertar a la izquierda
            ...
            cout<<"El elemento: "<<elemento.cantProds<<" ya se encuentra en el arbol"<<endl;
        }
    }
}
...
void imprimirNodo(NodoArbolBinarioBusqueda *nodo) {
    cout<<"("<<nodo->elemento.year<<"-";
    cout<<nodo->elemento.cantProds<<")"<<" ";
}
