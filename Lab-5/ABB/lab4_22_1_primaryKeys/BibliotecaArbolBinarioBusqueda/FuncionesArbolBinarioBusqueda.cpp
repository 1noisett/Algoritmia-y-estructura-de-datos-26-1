...
void insertarRecursivo(NodoArbolBinarioBusqueda *&raiz,const ElementoArbolBinarioBusqueda &elemento) {
    if (esNodoVacio(raiz)) {
        ...
        int cmp = comparaPK(raiz->elemento.campo1,raiz->elemento.campo2,elemento.campo1,elemento.campo2);
        if (cmp==1) {            // raiz mayor: insertar a la izquierda
            ...
            cout<<"El elemento: "<<elemento.campo1<<" ya se encuentra en el arbol"<<endl;
        }
    }
}

int comparaPK(int c1A, int c2A, int c1B, int c2B) {
    if (c1A > c1B) return 1;
    if (c1A < c1B) return -1;
    /* campo1 empatado: desempata campo2 */
    if (c2A > c2B) return 1;
    if (c2A < c2B) return -1;
    return 0; /* PK identica */
}

NodoArbolBinarioBusqueda *buscar(const ArbolBinarioBusqueda &arbol,const ElementoArbolBinarioBusqueda &elemento) {
    return buscarRecursivo(arbol.raiz,elemento);
}

NodoArbolBinarioBusqueda *buscarRecursivo(NodoArbolBinarioBusqueda *nodo,const ElementoArbolBinarioBusqueda &elemento) {
    if (esNodoVacio(nodo)) {
        return nullptr;
    } else {
        int cmp = comparaPK(nodo->elemento.campo1,nodo->elemento.campo2,elemento.campo1,elemento.campo2);
        if (cmp==0) {
            return nodo;
        } else if (cmp==1) {
...
void imprimirNodo(NodoArbolBinarioBusqueda *nodo) {
    cout<<"("<<nodo->elemento.campo1<<" "<<nodo->elemento.campo2<<" "<<nodo->elemento.campo3<<") ";
}
