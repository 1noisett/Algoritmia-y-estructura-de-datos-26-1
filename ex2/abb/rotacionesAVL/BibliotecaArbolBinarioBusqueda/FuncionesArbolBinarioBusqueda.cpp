...
NodoArbolBinarioBusqueda *insertarAVL(NodoArbolBinarioBusqueda *nodo,int clave) {
    if (esNodoVacio(nodo)) {
    ...
    if (FB>1 && clave<nodo->izq->elemento.dni) {
        return rotacionDerecha(nodo);
    }else if (FB<-1 && clave>nodo->der->elemento.dni) {
        return rotacionIzquierda(nodo);
    }else if (FB>1 && clave>nodo->izq->elemento.dni) {
