...
void insertarRecursivo(NodoArbolBinarioBusqueda *&raiz,const ElementoArbolBinarioBusqueda &elemento) {
    if (esNodoVacio(raiz)) {
        ...
        int cmp = comparaElementos(raiz->elemento.posicion,elemento.posicion);
        if (cmp==1) {            // raiz mayor: insertar a la izquierda
            ...
            cout<<"El elemento: "<<elemento.posicion<<" ya se encuentra en el arbol"<<endl;
        }
    }
}
...
bool buscarRecursivo(NodoArbolBinarioBusqueda *nodo,const ElementoArbolBinarioBusqueda &elemento) {
    if (esNodoVacio(nodo)) {
        ...
        int cmp = comparaElementos(nodo->elemento.posicion,elemento.posicion);
        if (cmp==0) {
            return nodo->elemento.caracter == elemento.caracter; // encontrada la posición: decide el carácter
        } else if (cmp==1) {
...
void imprimirNodo(NodoArbolBinarioBusqueda *nodo) {
    cout<<endl<<nodo->elemento.posicion<<nodo->elemento.caracter<<"  ";
}
